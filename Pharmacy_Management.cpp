...
class medicineOrder	//base class
{
public:
...
struct node //construct node
{
	...
	int x, menu2[10];
    double amount[10];
    string medicineName[10]={"Omee Tablet","Vitamin C(0.5g)","Ant-acid(500mg)","Multivitamin","Normet Tablet","Trysin Tablet",
	"Amino Zinc","Docid Tablet","Bitviz Tablet","Rhinosolvin"};
...
int main()	// Main function
{
	...
	system("COLOR B");		//Color to change background
	medicineOrder med;
	int menu;
	do
	{
		...
		case 2:
			{
				med.delete_order();	//function delete
				system("PAUSE");
				break;
			}	//end case 2
		...
		case 4:
			{
				med.order_list();	//function order
				system("PAUSE");
				break;
			}	//end case 4
		case 5:
			{
				med.daily_summary();	//function daily_summary
				system("PAUSE");
				break;
			}	//end case 5
        case 6:
			{
				med.exit();	//function exit
				goto a;
				break;
			}	//end case 6
		...
		default:
			{
				cout<<"You enter invalid input\re-enter the input\n"<<endl;
				break;
			}//end default
		}//end Switch
	}while(menu!=6);//end do
...
void medicineOrder::take_order() //scope resolution operator for defining function out of class
{
	system("cls");
	int i;
	...
	temp=new node;
				...
				cout <<"**************************************************************************\n";
                cout<<"0001"<<"\t"<<"\tOTC"<<"\t\t"<<"    Omee Tablet"<<"			RM 2.00"<<endl;
                cout<<"0002"<<"\t"<<"\tOTC"<<"\t\t"<<"    Vitamin C(500mg)"<<"		RM 3.00"<<endl;
                cout<<"0003"<<"\t"<<"\tOTC"<<"\t\t"<<"    Ant-acid(500mg)"<<"		RM 1.00"<<endl;
                cout<<"0004"<<"\t"<<"\tOTC"<<"\t\t"<<"    Multivitamin"<<"		RM 4.00"<<endl;
                cout<<"0005"<<"\t"<<"\tOTC"<<"\t\t"<<"    Normet Tablet"<<"	 	RM 1.00"<<endl;
                cout<<"0006"<<"\t"<<"\tOTC"<<"\t\t"<<"    Trycin Tablet"<<" 		RM 5.00"<<endl;
                cout<<"0007"<<"\t"<<"\tOTC"<<"\t\t"<<"    Amino Zinc "<<"		        RM 7.00"<<endl;
                cout<<"0008"<<"\t"<<"\tOTC"<<"\t\t"<<"    Docid Tablet"<<"	        RM 4.00"<<endl;//1353fn
                cout<<"0009"<<"\t"<<"\tOTC"<<"\t\t"<<"    Bizvit Tablet"<<"		RM 3.00"<<endl;
                cout<<"0010"<<"\t"<<"\tOTC"<<"\t\t"<<"    Rhinosolvin"<<"		        RM 5.00"<<endl;
                cout<<" "<<endl;
	...
	for (i=0; i<temp->x; i++)
	{
		...
		cout << "\n"<<i+1<<"."<<"Please enter your selection : "<<endl;
		cin>> temp->menu2[i];
...
void medicineOrder::order_list()		//Function to display receipt
{
	int i, num, num2;
	...
	while(temp !=NULL && !found) //when the linked list is not empty
	{
		if (temp->reciept_number==num2) //comparing the receipt number from the user to the already existing receipt number
		{
			found = true;
		}
		else
...
void medicineOrder::delete_order()	//function to delete_order
{
	system("cls");
	int i, num, count;
...
void medicineOrder::modify()		//function to modify order
{
 system("cls");
 int i, ch, sid;
 bool found;
...
void medicineOrder::daily_summary()		//Function to display Daily Summary
{
	int i,num;
	system("cls");
	node *temp ;
...
void medicineOrder::exit() //Function to exit
{
	cout<<"\nYou choose to exit.\n"<<endl;
}	//end function exit
